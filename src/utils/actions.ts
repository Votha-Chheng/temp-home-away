...
const getAuthUser = async()=> {
  ...
  if(!user) throw new Error("Vous devez êtres connecté pour accéder à la page demandée.")
...
export const createProfileAction = async (prevState: any, formData: FormData)=> {
  try {
    const user = await currentUser()
    if(!user) throw new Error("Connectez-vous pour créer un profil.")
...
export const updateProfileAction = async (prevState: any, formData: FormData): Promise<{message:string}>=> {
  ...
  try {
    ...
    return { message: "Le profil a bien été mis à jour."}
...
export const updateProfileImageAction = async (prevState: any, formData: FormData): Promise<{message:string}>=> {
  ...
  try {
    ...
    revalidatePath("/profile")
    return { message: "L'image a été mise à jour."}
...
export const createPropertyAction = async(prevState: any, formData: FormData): Promise<{message:string}>=> {
  ...
  try {
    ...
    const fullpPath = await uploadImage(validatedFile.image)
...
export const toggleFavoriteAction = async(prevState: { propertyId: string, favoriteId: string|null, pathname: string})=> {
  ...
  try {
    ...
    return {
      message: favoriteId ? "Supprimé des favoris":"Ajouté aux favoris"
    }
  } catch (error) {
    return renderError(error)
  }
